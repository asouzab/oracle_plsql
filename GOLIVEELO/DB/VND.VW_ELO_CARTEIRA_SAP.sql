...
/* Formatted on 17/05/2018 14:58:32 (QP5 v5.318) */
CREATE OR REPLACE FORCE VIEW VND.VW_ELO_CARTEIRA_SAP
(
    CD_ELO_CARTEIRA_SAP,
    NU_CARTEIRA_VERSION,
    CD_CENTRO_EXPEDIDOR,
...
PB.CD_SALES_DISTRICT, PB.NO_SALES_DISTRICT, 
PB.CD_SALES_OFFICE, PB.NO_SALES_OFFICE,
PB.CD_SALES_GROUP, PB.NO_SALES_GROUP

FROM VND.VW_ELO_CARTEIRA_SAP_SALESGROUP PB
WHERE PB.NO_SALES_GROUP IS NOT NULL AND PB.NO_SALES_GROUP <> ' ' AND LENGTH (PB.NO_SALES_GROUP) > 3
UNION 
SELECT DISTINCT 
PB.NU_CARTEIRA_VERSION,
PB.CD_SALES_DISTRICT, PB.NO_SALES_DISTRICT, 
...
WHERE 
PB.DH_CARTEIRA > (SYSDATE - 30)

)


SELECT CTSAP.CD_ELO_CARTEIRA_SAP,
     ...
     WHERE    CTSAP.DH_CARTEIRA > (SYSDATE - 30)
           OR EXISTS
                  (SELECT 1
                     FROM VND.ELO_AGENDAMENTO AGE
                    WHERE     AGE.NU_CARTEIRA_VERSION =
                              CTSAP.NU_CARTEIRA_VERSION
                          AND AGE.CD_ELO_STATUS IN (1,
                                                    2,
                                                    3,
                                                    4,
                                                    5,
...
GRANT SELECT ON VND.VW_ELO_CARTEIRA_SAP TO CTF WITH GRANT OPTION;

GRANT SELECT ON VND.VW_ELO_CARTEIRA_SAP TO VND_SEC WITH GRANT OPTION;