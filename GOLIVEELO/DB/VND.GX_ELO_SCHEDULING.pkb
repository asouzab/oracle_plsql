CREATE OR REPLACE PACKAGE BODY VND."GX_ELO_SCHEDULING" IS
    ...
    BEGIN
        ...
        SELECT DISTINCT
               ...
               CASE 
                    ...
                    WHEN ea.cd_machine IS NOT NULL THEN 'Máquina: ' || ea.cd_machine || ' - ' || ma.ds_machine
               END site,
               CASE 
                    WHEN ea.cd_polo IS NOT NULL THEN 'Polo: ' || ea.cd_polo || ' - ' || po.ds_polo
                    WHEN ea.cd_centro_expedidor IS NOT NULL THEN 'Centro: ' || ea.cd_centro_expedidor || ' - ' || ce.ds_centro_expedidor
               ...
               END SiteType,
               po.ds_polo               DsPolo,
               ce.ds_centro_expedidor   DsCenter,
               ma.ds_machine            DsMachine
...
--               NVL(total_planned_quantity.wd_total_planned_quantity, 0) AS wd_total_planned_quantity,
        ...
        IF NVL(p_qt_semana, 0) = 0 THEN
            ...
            IF v_cd_tipo_agendamento = v_cd_tipo_replan THEN
                ...
                UPDATE vnd.elo_carteira ca
				   ...
				   DS_VERSAO = SUBSTR(NVL(DS_VERSAO, ' ') ||  '[{"ID": 0001, "APP": "GX_ELO_SCHEDULING.pi_agendamento_week_qty", "PROPERTIE": [{"NAME": "QT_AGENDADA_CONFIRMADA", "VAL": NULL}], "DH_ULT_MOD": ' || TO_CHAR(CURRENT_DATE) || ' }],' ,1, 4000)
    ...
    IS
        ...
        v_total                     vnd.elo_agendamento_day.nu_quantidade%TYPE := 0;
        v_cd_elo_agendamento_day    vnd.elo_agendamento_day.cd_elo_agendamento_day%TYPE;
    ...
    BEGIN
        ...
        -- Correcao de arredondamento.
        BEGIN
            SELECT SUM(ad.nu_quantidade)
              INTO v_total
              FROM vnd.elo_agendamento_day ad
             INNER JOIN
                   vnd.elo_agendamento_week aw
                   on aw.cd_elo_agendamento_week = ad.cd_elo_agendamento_week
             WHERE ad.cd_grupo_embalagem = p_cd_grupo_embalagem
               AND aw.cd_elo_agendamento_item = p_cd_elo_agendamento_item
            ;
        EXCEPTION  
            WHEN OTHERS THEN v_total := p_nu_quantidade;
        END;
        
        IF v_total < p_nu_quantidade THEN
            BEGIN
                SELECT cd_elo_agendamento_day
                  ...
                  FROM (
                            SELECT *
                             ...
                             WHERE ad.cd_grupo_embalagem = p_cd_grupo_embalagem
                               AND aw.cd_elo_agendamento_item = p_cd_elo_agendamento_item
                               AND NVL(ad.nu_quantidade, 0) > 0
                             ORDER BY ad.nu_quantidade
                       )
                   WHERE ROWNUM = 1
                ;
            EXCEPTION  
                WHEN OTHERS THEN v_cd_elo_agendamento_day := 0;
            END;
            
            UPDATE vnd.elo_agendamento_day
               SET nu_quantidade = nu_quantidade + (p_nu_quantidade - v_total)
             WHERE cd_elo_agendamento_day = v_cd_elo_agendamento_day;
             
            COMMIT;
        END IF;
        -- Fim da correcao de arredondamento.
...
--        );
    ...
    IS
        ...
        v_count                     NUMBER := 0;
    BEGIN
        IF p_qt_agendada = 0 THEN
            DELETE FROM vnd.elo_agendamento_grouping
             WHERE nu_documento = p_nu_documento
               AND sg_tipo_documento = p_sg_tipo_documento
            ;
            COMMIT;
        ELSE
            BEGIN
    ...
    IS
        ...
        V_AGEND_AGNEW VND.ELO_STATUS.CD_ELO_STATUS%type  ;
        V_AGEND_AGOPN VND.ELO_STATUS.CD_ELO_STATUS%type  ;        
    ...
    BEGIN
        ...
        V_AGEND_AGNEW:=  VND.GX_ELO_COMMON.fx_elo_status('AGEND', 'AGNEW');
        V_AGEND_AGOPN:=  VND.GX_ELO_COMMON.fx_elo_status('AGEND', 'AGOPN');
        ...
        SELECT an.cd_elo_agendamento,
          ...
          WHERE 
            ...
            AND an.ic_ativo = 'S'
            AND an.CD_ELO_STATUS in (V_AGEND_AGNEW,V_AGEND_AGOPN )
        ;
		EXCEPTION 
         WHEN NO_DATA_FOUND THEN 
			BEGIN
...
--        SELECT gx_elo_common.fx_elo_status_order('AGEND', 'AGFIN')
...
--        END IF;
          ...
          IF NVL(v_cd_elo_agendamento, 0) = 0 THEN
                RETURN;
          END IF;
...
--                ;
    ...
    IS
        ...
        v_cd_elo_carteira           vnd.elo_carteira.cd_elo_carteira%TYPE;
        v_cd_elo_agendamento_item   vnd.elo_agendamento_item.cd_elo_agendamento_item%TYPE;
        v_qt_saldo_by_item          vnd.elo_carteira.qt_saldo%TYPE;
        v_qt_semana_item            vnd.elo_agendamento_week.qt_semana%TYPE;
        v_qt_saldo_semana           vnd.elo_agendamento_week.qt_semana%TYPE;
        v_count_semana_disponivel   NUMBER;
        v_limit                     NUMBER := 10000;
        ...
        v_qt_semana_by_item VND.ELO_AGENDAMENTO_WEEK.QT_SEMANA%TYPE;
        v_qt_consumido_by_item VND.ELO_CARTEIRA.QT_AGENDADA%TYPE;        
...
--                      aw.qt_semana
        ...
        (
            ...
            qt_saldo                        vnd.elo_carteira.qt_saldo%TYPE,
            nu_ordem                        vnd.elo_carteira.nu_ordem%TYPE,
            qt_saldo_by_item             vnd.elo_carteira.qt_saldo%TYPE,
        ...
        filter_item                 elo_agendamento_item_t;
        tof_carteira_to_day_prog    elo_agendamento_item_t;           
        
        
        tof_todo_carteira_day carteira_days_t;
        ...
        NVL((SELECT NVL(SUM(aw.qt_semana), 0)
         ...
         WHERE --NVL(ca.qt_saldo, 0) - NVL(ca.qt_agendada, 0) > 0
           --AND NVL(aw.qt_semana, 0) > 0
           ca.cd_elo_agendamento = v_cd_elo_agendamento
           AND ca.ic_ativo = 'S'
           AND ai.ic_ativo = 'S'
             and (p_cd_sales_office IS NULL OR ap.cd_sales_office = p_cd_sales_office)
               AND (p_cd_sales_group IS NULL OR ap.cd_sales_group = p_cd_sales_group)           
        ...
        (
            ...
            nu_dia_semana           vnd.elo_agendamento_day.nu_dia_semana%TYPE,
            qt_agendada_confirmada  vnd.elo_carteira.qt_agendada_confirmada%TYPE,
            qt_agendamento          vnd.elo_carteira_day.nu_quantidade%TYPE,
            qt_disponivel           vnd.elo_carteira.qt_agendada_confirmada%TYPE,
            qt_disponivel_by_day    vnd.elo_carteira.qt_agendada_confirmada%TYPE
        );
        TYPE carteira_day_t IS TABLE OF carteira_day_r;
        t_carteira_day carteira_day_t;

        CURSOR c_carteira_day 
        (pi_cd_elo_agendamento_item vnd.elo_agendamento_item.cd_elo_agendamento_item%TYPE)
        ...
        and AD.NU_DIA_SEMANA = EMBINC.NU_DIA_SEMANA                
         ...
         ai.cd_elo_agendamento_item = pi_cd_elo_agendamento_item
...
--        END IF;
		...
		*/
        ...
        LOOP
            ...
            EXIT WHEN v_count_semana_disponivel = 0 OR v_count >= V_CART_QT_LINHAS;

            --ordenado by elo_agendamento_item and nu_ordem (always to work)

            --V_LINHA:=1;
            v_item_new:=t_carteira(V_LINHA).cd_elo_agendamento_item;
            v_item_old:=v_item_new; 
            
            v_qt_semana_by_item:= t_carteira(V_LINHA).qt_semana_by_item;    
            --   v_qt_semana_by_item this is total quantity of week by item  (maximum qt)     
            v_qt_saldo_by_item:= nvl(t_carteira(V_LINHA).qt_saldo_by_item,0);
            --   v_qt_saldo_by_item is almost the same v_qt_semana_by_item (sum carteira qt saldo, this total is maximum qt)
            
            if v_qt_saldo_by_item > v_qt_semana_by_item then
                v_qt_saldo_by_item:=v_qt_semana_by_item;
            end if ;
            ...
            LOOP
                ...
                IF t_carteira(V_LINHA).qt_saldo >= v_qt_saldo_by_item THEN
                    BEGIN
                    v_qt_agendada := v_qt_saldo_by_item;
                    v_qt_saldo_by_item:=0;
                    END;
                ELSE
                    v_qt_agendada := t_carteira(V_LINHA).qt_saldo;
                    v_qt_saldo_by_item:=v_qt_saldo_by_item - v_qt_agendada;
                END IF;

--                ;

                tof_carteira_vol_prog(tof_carteira_vol_prog.COUNT+1).CD_ELO_CARTEIRA:= t_carteira(V_LINHA).cd_elo_carteira;
                tof_carteira_vol_prog(tof_carteira_vol_prog.COUNT).CD_ELO_AGENDAMENTO_ITEM:= t_carteira(V_LINHA).CD_ELO_AGENDAMENTO_ITEM; 
                tof_carteira_vol_prog(tof_carteira_vol_prog.COUNT).qt_agendada:= v_qt_agendada; 
                tof_carteira_vol_prog(tof_carteira_vol_prog.COUNT).qt_agendada_confirmada:= v_qt_agendada; 
...
--region pu_end_supervisor
      
        
        IF tof_carteira_vol_prog.COUNT > 0 THEN 
        BEGIN
        
            FOR i_cart in tof_carteira_vol_prog.FIRST .. tof_carteira_vol_prog.LAST
            LOOP
                filter_item(filter_item.COUNT+1).cd_elo_agendamento_item:= tof_carteira_vol_prog(i_cart).cd_elo_agendamento_item; 
            END LOOP;
        
            tof_carteira_to_day_prog:= filter_item ; 
        ...
        FORALL i_cart in INDICES OF tof_carteira_vol_prog
                UPDATE vnd.elo_carteira ec
                   SET ec.qt_agendada = tof_carteira_vol_prog(i_cart).qt_agendada,
                       ec.qt_agendada_confirmada = tof_carteira_vol_prog(i_cart).qt_agendada_confirmada
                 WHERE ec.cd_elo_carteira = tof_carteira_vol_prog(i_cart).cd_elo_carteira
                ;
				COMMIT;        
        EXCEPTION  
        WHEN OTHERS THEN 
        BEGIN
            RAISE_APPLICATION_ERROR(-20001, 'ERRO ENCONTRADO: GX_ELO_SCHEDULING.099 - ' || SQLCODE || ' -ERROR- ' || SQLERRM);
            ROLLBACK;
        END;    
        END;        
        
    V_CART_QT_LINHAS:=tof_carteira_to_day_prog.COUNT;
    
    IF V_CART_QT_LINHAS > 0 THEN 
    BEGIN

            FOR i_cart IN tof_carteira_to_day_prog.First .. tof_carteira_to_day_prog.Last
            --<<carteiramaster>>
            LOOP

                v_qt_agendada:=0;--tof_carteira_to_day_prog(i_cart).qt_agendada;

                -- Inserts or updates ELO_CARTEIRA_DAY.
                OPEN    c_carteira_day(tof_carteira_to_day_prog(i_cart).cd_elo_agendamento_item);                               
                FETCH   c_carteira_day BULK COLLECT INTO t_carteira_day LIMIT v_limit;
                CLOSE   c_carteira_day;

                CONTINUE WHEN t_carteira_day.COUNT = 0;

                --<<day_loop>>
               -- LOOP
                    --<<carteira_day_loop>>
                    FOR i_cart_day IN t_carteira_day.FIRST .. t_carteira_day.LAST
                    LOOP

						BEGIN	
						
						tof_todo_carteira_day(tof_todo_carteira_day.COUNT+1).cd_elo_carteira:= t_carteira_day(i_cart_day).cd_elo_carteira;
						tof_todo_carteira_day(tof_todo_carteira_day.COUNT).cd_grupo_embalagem:=t_carteira_day(i_cart_day).cd_grupo_embalagem;
						tof_todo_carteira_day(tof_todo_carteira_day.COUNT).nu_semana:=t_carteira_day(i_cart_day).nu_semana;
						tof_todo_carteira_day(tof_todo_carteira_day.COUNT).nu_dia_semana:=t_carteira_day(i_cart_day).nu_dia_semana;
						tof_todo_carteira_day(tof_todo_carteira_day.COUNT).nu_quantidade:=t_carteira_day(i_cart_day).qt_disponivel_by_day;
		...
		BEGIN
        gx_elo_scheduling.pu_carteira_day( list_of_carteira_day => tof_todo_carteira_day    );
		EXCEPTION  
		WHEN OTHERS THEN 
		BEGIN
			RAISE_APPLICATION_ERROR(-20001, 'ERRO ENCONTRADO: GX_ELO_SCHEDULING.057 - ' || SQLCODE || ' -ERROR- ' || SQLERRM);
...
--end region          
    ...
    IS
        ...
        SELECT ca.cd_elo_carteira,
          ...
          FROM vnd.elo_carteira ca
         WHERE --NVL(ca.qt_saldo, 0) - NVL(ca.qt_agendada, 0) > 0
           ca.cd_elo_agendamento = p_cd_elo_agendamento --332
           --AND ca.cd_elo_agendamento_item = v_cd_elo_agendamento_item --45868
           AND ca.ic_ativo = 'S'
           --AND ca.ic_emergencial = 'S'
           AND EXISTS (SELECT 1 FROM vnd.elo_agendamento_supervisor ap
           INNER JOIN vnd.elo_agendamento_item ai
               ON ai.cd_elo_agendamento_supervisor = ap.cd_elo_agendamento_supervisor
           INNER JOIN  VND.ELO_AGENDAMENTO_WEEK aws
           ON ai.cd_elo_agendamento_item = aws.cd_elo_agendamento_item
        ...
        TYPE carteira_emergencial_r IS RECORD
        (
            cd_elo_carteira                 vnd.elo_carteira.cd_elo_carteira%TYPE,
            cd_elo_agendamento_item         vnd.elo_agendamento_item.cd_elo_agendamento_item%TYPE,
            qt_emergencial                  vnd.elo_carteira.qt_emergencial%TYPE,
            ic_emergencial                  vnd.elo_carteira.ic_emergencial%TYPE
        );
        TYPE carteira_emergencial_t IS TABLE OF carteira_emergencial_r
         INDEX BY PLS_INTEGER;         
         
        tofcarteira_emergencial_t carteira_emergencial_t;
    ...
    BEGIN
            ...
            LOOP
                ...
                tofcarteira_emergencial_t(tofcarteira_emergencial_t.COUNT +1).cd_elo_carteira:= t_carteira(j).cd_elo_carteira;
                tofcarteira_emergencial_t(tofcarteira_emergencial_t.COUNT).cd_elo_agendamento_item:= t_carteira(j).cd_elo_agendamento_item;
                tofcarteira_emergencial_t(tofcarteira_emergencial_t.COUNT).qt_emergencial:= t_carteira(j).qt_saldo;
                
                if t_carteira(j).qt_saldo > 0 then 
                    tofcarteira_emergencial_t(tofcarteira_emergencial_t.COUNT).ic_emergencial:= 'S';
                else 
                    tofcarteira_emergencial_t(tofcarteira_emergencial_t.COUNT).ic_emergencial:= 'N';
                end if;
            END LOOP;            
            ...
            FORALL i_cart in INDICES OF tofcarteira_emergencial_t
            UPDATE vnd.elo_carteira ec
               SET ec.qt_emergencial = tofcarteira_emergencial_t(i_cart).qt_emergencial
               , ic_emergencial =  tofcarteira_emergencial_t(i_cart).ic_emergencial
              
             WHERE ec.cd_elo_carteira = tofcarteira_emergencial_t(i_cart).cd_elo_carteira;
...
--        END IF;
        ...
        BEGIN 
        SELECT NVL(SUM(aw.qt_semana), 0), COUNT(aw.cd_elo_agendamento_week)
          INTO v_qt_semana, v_count
          FROM vnd.elo_agendamento_week aw
         WHERE aw.cd_elo_agendamento_item = p_cd_elo_agendamento_item
        ;
        EXCEPTION  
        ...
        IF NVL(v_qt_semana,0) = 0 or NVL(v_count,0) = 0 THEN
        BEGIN
            ...
            BEGIN
                ...
                ec.ds_versao = SUBSTR(NVL(ec.ds_versao, ' ')
                || '['
                ||     '{"ID": 0005, "APP": "GX_ELO_SCHEDULING.PU_DISTRIBUTE_ITEM",' 
                ||     '"PROPERTIES": ['
                ||         '{"NAME": "QT_AGENDADA_CONFIRMADA", "VAL": 0' ||   '},'
                ||         '{"NAME": "QT_AGENDADA", "VAL": 0' ||  '},'
                ||         '{"NAME": "QT_SEMANA", "VAL": 0' ||  '},'                                                    
                ...
                WHEN OTHERS THEN 
                BEGIN
                RAISE_APPLICATION_ERROR(-20001, 'ERRO ENCONTRADO: GX_ELO_SCHEDULING.069a - ' || SQLCODE || ' -ERROR- ' || SQLERRM);
                ROLLBACK;
                END; 
            END;
                ...
                UPDATE vnd.elo_carteira ec
                   SET ec.qt_agendada               = NVL(ec.qt_agendada, 0) + v_qt_agendada,
                       ...
                       ec.ds_versao                 = SUBSTR(NVL(ec.ds_versao, ' ')
                                                             ...
                                                             ||     '{"ID": 0005, "APP": "GX_ELO_SCHEDULING.PU_DISTRIBUTE_ITEM",' 
                                                             ||     '"PROPERTIES": ['
                                                             ||         '{"NAME": "QT_AGENDADA_CONFIRMADA", "VAL":' || NVL(TO_CHAR(NVL(ec.qt_agendada, 0) + v_qt_agendada), 'NULL') || '},'
                                                             ||         '{"NAME": "QT_AGENDADA", "VAL":' || NVL(TO_CHAR(NVL(ec.qt_agendada, 0) + v_qt_agendada), 'NULL') ||  '},'                               
                                                             ||     '], "DH_ULT_MOD": ' || TO_CHAR(CURRENT_DATE, 'DD/MM/RRRR HH24:MI:SS') || ' }'
                                                             || '],' ,1 , 4000)   
		...
		BEGIN
            UPDATE vnd.elo_agendamento_item ai
               SET ai.ds_versao = SUBSTR(NVL(ai.ds_versao, ' ') 
                                         ||  '['
                                         ||       '{"ID": 0005, '
                                         ||       '"APP": "GX_ELO_SCHEDULING.PU_DISTRIBUTE_ITEM",' 
                                         ||       '"DH_ULT_MOD": ' || TO_CHAR(CURRENT_DATE, 'DD/MM/RRRR HH24:MI:SS') || ' }'
                                         ||  '],' ,1 , 4000) 
             WHERE ai.cd_elo_agendamento_item = p_cd_elo_agendamento_item
            ;
		...
		EXCEPTION  
            WHEN OTHERS THEN 
                BEGIN
                    RAISE_APPLICATION_ERROR(-20001, 'ERRO ENCONTRADO: GX_ELO_SCHEDULING.073 - ' || SQLCODE || ' -ERROR- ' || SQLERRM);
                    ROLLBACK;
                END; 
		END;  
    ...
    IS
        ...
        CURSOR C_AGEND IS 
        SELECT AGE_SUP.CD_ELO_AGENDAMENTO_SUPERVISOR  
        FROM VND.ELO_AGENDAMENTO AGE
        INNER JOIN VND.ELO_AGENDAMENTO_SUPERVISOR AGE_SUP
        ON 
        AGE.CD_ELO_AGENDAMENTO = AGE_SUP.CD_ELO_AGENDAMENTO 
        WHERE 
        AGE.CD_ELO_AGENDAMENTO =  p_cd_elo_agendamento
        AND (p_cd_sales_office IS NULL OR AGE_SUP.cd_sales_office = p_cd_sales_office)
        AND (p_cd_sales_group IS NULL OR AGE_SUP.cd_sales_group = p_cd_sales_group)
        ;

        TYPE supervisor_r IS RECORD
        (
            CD_ELO_AGENDAMENTO_SUPERVISOR                VND.ELO_AGENDAMENTO_SUPERVISOR.CD_ELO_AGENDAMENTO_SUPERVISOR%TYPE
        );

        TYPE supervisor_t IS TABLE OF supervisor_r;
        tof_supervisor supervisor_t;
        V_ELO_ST_ASFIN  VND.ELO_AGENDAMENTO_SUPERVISOR.CD_ELO_STATUS%TYPE;
    ...
    BEGIN
        ...
        V_ELO_ST_ASFIN:=vnd.gx_elo_common.fx_elo_status('AGSUP', 'ASFIN');
        
        OPEN C_AGEND;
        FETCH C_AGEND BULK COLLECT INTO tof_supervisor LIMIT 100;
        CLOSE C_AGEND;        
        
        FORALL C_LINHA IN tof_supervisor.FIRST .. tof_supervisor.LAST SAVE EXCEPTIONS
        UPDATE vnd.elo_agendamento_supervisor ap
           SET ap.cd_elo_status = V_ELO_ST_ASFIN
         WHERE ap.cd_elo_agendamento_supervisor = tof_supervisor(C_LINHA).CD_ELO_AGENDAMENTO_SUPERVISOR
        ...
        BEGIN
            ...
            LOOP
            RAISE_APPLICATION_ERROR(-20001, 'ERRO ENCONTRADO: GX_ELO_SCHEDULING.333 - ' ||
        ...
        SELECT COUNT(cd_elo_agendamento_supervisor)
         ...
         WHERE ap.cd_elo_agendamento = p_cd_elo_agendamento
           AND ap.cd_elo_status <> V_ELO_ST_ASFIN
           AND ap.ic_ativo = 'S'
        ;
        EXCEPTION 
        WHEN NO_DATA_FOUND THEN 
...
--               SET nu_quantidade       = nu_quantidade + p_nu_quantidade
    ...
    PROCEDURE pu_carteira_day (
        list_of_carteira_day  IN  carteira_days_t
    )
    IS
    BEGIN
		BEGIN
		
		FORALL C_LINHA IN INDICES OF list_of_carteira_day
		
        MERGE INTO vnd.elo_carteira_day USING DUAL ON (
                cd_elo_carteira     = list_of_carteira_day(C_LINHA).cd_elo_carteira
            AND cd_grupo_embalagem  = list_of_carteira_day(C_LINHA).cd_grupo_embalagem
            AND nu_semana           = list_of_carteira_day(C_LINHA).nu_semana
            AND nu_dia_semana       = list_of_carteira_day(C_LINHA).nu_dia_semana
        )

        WHEN MATCHED THEN
            UPDATE 
--               SET nu_quantidade       = nu_quantidade + p_nu_quantidade
               SET nu_quantidade       = list_of_carteira_day(C_LINHA).nu_quantidade
             WHERE cd_elo_carteira     = list_of_carteira_day(C_LINHA).cd_elo_carteira
               AND cd_grupo_embalagem  = list_of_carteira_day(C_LINHA).cd_grupo_embalagem
               AND nu_semana           = list_of_carteira_day(C_LINHA).nu_semana
               AND nu_dia_semana       = list_of_carteira_day(C_LINHA).nu_dia_semana
        ...
        WHEN NOT MATCHED THEN
            INSERT (
                ...
                seq_elo_carteira_day.nextval,
                list_of_carteira_day(C_LINHA).cd_elo_carteira, 
                list_of_carteira_day(C_LINHA).cd_grupo_embalagem, 
                list_of_carteira_day(C_LINHA).nu_semana,
                list_of_carteira_day(C_LINHA).nu_dia_semana, 
                list_of_carteira_day(C_LINHA).nu_quantidade
            ) WHERE list_of_carteira_day(C_LINHA).nu_quantidade > 0  ;
			COMMIT;
            EXCEPTION 
            WHEN OTHERS THEN
			BEGIN
    ...
    BEGIN
        ...
        SELECT NVL(COUNT(ci.cd_agendamento_centro_item), 0) Quantity,
               ag.cd_week           AS WeekCode,
               ag.dt_week_start     AS WeekStartDate,
               p_site_type          AS SiteType,
               p_cd_site            AS SiteId
          FROM vnd.elo_agendamento ag
