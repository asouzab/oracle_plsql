...
where cd_elo_agendamento = 723create or replace PACKAGE BODY     "GX_ELO_AGENDAMENTO" IS
        ...
        (
            SELECT DISTINCT
             ...
             WHERE
                    ...
                    AND CA.SG_DESTINO_BACKLOG_CIF = 'CSHIP'
                    AND CA.IC_ATIVO = 'S'
                     ...
                     SELECT DISTINCT ca.cd_sales_group,
                            ...
                            */
                            CASE WHEN us_nac.cd_usuario_original IS NULL THEN 
                                      gx_elo_agendamento.fx_latest_sales_district(p_cd_elo_agendamento, ca.cd_sales_office)
                                 ELSE 
...
select * from ELO_AGENDAMENTOSTATUS_LOG
where cd_elo_agendamento = 32

select * from vnd.elo_status 
WHERE SG_STATUS = 'ASFIN'

SELECT * FROM VND.ELO_TIPO_STATUS 
WHERE CD_ELO_TIPO_STATUS = 7

select * from v$session 
13955225

select * from vnd.elo_agendamento 
where cd_week = 'W182018' AND  CD_POLO = 'P002'
SELECT * FROM CTF.POLO

SELECT * FROM VND.ELO_AGENDAMENTO_DAY WHERE CD_ELO_AGENDAMENTO_WEEK IN (
SELECT W.CD_ELO_AGENDAMENTO_WEEK , W.* FROM VND.ELO_AGENDAMENTO_WEEK W WHERE W.CD_ELO_AGENDAMENTO_ITEM IN (
select CD_ELO_AGENDAMENTO_ITEM from vnd.elo_agendamento_item where cd_elo_agendamento_supervisor in (
select cd_elo_agendamento_supervisor from vnd.elo_agendamento_supervisor where cd_elo_agendamento = 24)
and cd_cliente = '0004025225' and cd_incoterms = 'CIF'
)
)

SELECT * FROM VND.ELO_AGENDAMENTO_WEEK WHERE CD_ELO_AGENDAMENTO_WEEK = 1483
CD_ELO_AGENDAMENTO_ITEM = 2444

SELECT C.QT_AGENDADA_CONFIRMADA, C.*  FROM VND.ELO_CARTEIRA C 
WHERE 
c.cd_elo_agendamento = 45
and c.qt_AGENDADA_CONFIRMADA >0
--c.CD_CLIENTE LIKE '%0004025225%'
C.CD_ELO_AGENDAMENTO_ITEM = 5381

SELECT * FROM VND.VW_ELO_AGENDAMENTO_WEEK_PLAN 
WHERE CD_ELO_AGENDAMENTO_ITEM = 2166

SELECT * FROM VND.VW_ELO_AGENDAMENTO_DAY_PLAN 
WHERE CD_ELO_AGENDAMENTO_WEEK = 2166

SELECT * FROM VND.ELO_AGENDAMENTO_DAY 
WHERE CD_ELO_AGENDAMENTO_WEEK = 2166


select * from update vnd.elo_agendamento 


update vnd.elo_agendamento 
set --dh_limite = sysdate + 10
cd_elo_status = 2
where cd_elo_agendamento = 723;

                    UPDATE VND.ELO_CARTEIRA CT
                    SET CT.CD_STATUS_LOGISTICA = 10,
                        CT.CD_STATUS_CUSTOMER_SERVICE = 10

                    WHERE   CT.QT_AGENDADA_CONFIRMADA > 0 
                    AND CT.CD_STATUS_CUSTOMER_SERVICE IS NULL
                    AND     CT.CD_ELO_AGENDAMENTO = 45;

select * from vnd.elo_carteira ct
 WHERE   CT.QT_AGENDADA_CONFIRMADA > 0 
                    AND CT.CD_STATUS_CUSTOMER_SERVICE IS not NULL
                    and ct.cd_tipo_agendamento is not null
                    AND     CT.CD_ELO_AGENDAMENTO = 45;
                    
select * from vnd.elo_agendamento 
where 
cd_elo_agendamento = 45


select * from all_objects
where status = 'VALID'
and last_ddl_time > sysdate-5
and owner = 'VND' AND OBJECT_NAME LIKE '%ELO%'
