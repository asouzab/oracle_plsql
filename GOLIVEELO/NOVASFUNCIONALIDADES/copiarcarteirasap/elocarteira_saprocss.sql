...
CREATE OR REPLACE PROCEDURE vnd.PI_ELO_CARTEIRA_SAP_IMPORT 
(
...
--P_NU_CARTEIRA_VERSION VND.ELO_CARTEIRA_SAP.NU_CARTEIRA_VERSION%TYPE:='20180731041552';
V_NU_CARTEIRA_VERSION VND.ELO_CARTEIRA_SAP.NU_CARTEIRA_VERSION%TYPE:=NULL;
V_TRAVA            VARCHAR2(1):='I';


CURSOR VERS ( PI_NU_CARTEIRA_VERSION VND.ELO_CARTEIRA_SAP.NU_CARTEIRA_VERSION%TYPE )
IS 
...
WHERE 1=1
AND sap.NU_CARTEIRA_VERSION = PI_NU_CARTEIRA_VERSION
...
CURSOR C_SALES_STRU (PI_NU_CARTEIRA_VERSION VND.ELO_CARTEIRA_SAP.NU_CARTEIRA_VERSION%TYPE) IS  
...
PB.CD_SALES_GROUP,
PB.NO_SALES_GROUP
FROM CTE_SALES_GROUP_INI PB
WHERE PB.NU_CARTEIRA_VERSION =  PI_NU_CARTEIRA_VERSION
;


TYPE elo_sales_group_r IS RECORD
(
NU_CARTEIRA_VERSION     VND.ELO_CARTEIRA_SAP.NU_CARTEIRA_VERSION%TYPE,
CD_SALES_DISTRICT       VND.ELO_CARTEIRA_SAP.CD_SALES_DISTRICT%TYPE,
NO_SALES_DISTRICT       VND.ELO_CARTEIRA_SAP.NO_SALES_DISTRICT%TYPE,
CD_SALES_OFFICE         VND.ELO_CARTEIRA_SAP.CD_SALES_OFFICE%TYPE,
NO_SALES_OFFICE         VND.ELO_CARTEIRA_SAP.NO_SALES_OFFICE%TYPE,
CD_SALES_GROUP          VND.ELO_CARTEIRA_SAP.CD_SALES_GROUP%TYPE,
NO_SALES_GROUP          VND.ELO_CARTEIRA_SAP.NO_SALES_GROUP%TYPE

);

TYPE elo_sales_group_t IS TABLE OF elo_sales_group_r
INDEX BY PLS_INTEGER;
tof_sales_group elo_sales_group_t;

CURSOR C_PRODUTOSAP (PI_NU_CARTEIRA_VERSION VND.ELO_CARTEIRA_SAP.NU_CARTEIRA_VERSION%TYPE) IS  

WITH CTE_PRODUTO_BRL AS 
(
SELECT 
GRP.NU_CARTEIRA_VERSION,
GRP.CD_SALES_ORG, 
GRP.CD_PRODUTO_SAP, 
MAX(GRP.NO_PRODUTO_SAP) NO_PRODUTO_SAP
FROM VND.ELO_CARTEIRA_SAP GRP
WHERE GRP.CD_MOEDA  = 'BRL'
AND GRP.NU_CARTEIRA_VERSION = PI_NU_CARTEIRA_VERSION
GROUP BY 
GRP.NU_CARTEIRA_VERSION,
GRP.CD_SALES_ORG, 
GRP.CD_PRODUTO_SAP
...
GRP.CD_PRODUTO_SAP,
GRP.NO_PRODUTO_SAP 
FROM CTE_PRODUTO_BRL GRP

UNION 

SELECT
XGRP.NU_CARTEIRA_VERSION, 
XGRP.CD_SALES_ORG,
XGRP.CD_PRODUTO_SAP, 
MAX(XGRP.NO_PRODUTO_SAP) NO_PRODUTO_SAP
FROM VND.ELO_CARTEIRA_SAP XGRP
WHERE 
XGRP.NU_CARTEIRA_VERSION = PI_NU_CARTEIRA_VERSION
AND NOT EXISTS (SELECT DISTINCT 1 
                    FROM CTE_PRODUTO_BRL TR 
                    WHERE TR.CD_SALES_ORG = XGRP.CD_SALES_ORG
                    AND TR.NU_CARTEIRA_VERSION  = XGRP.NU_CARTEIRA_VERSION
...
TYPE elo_produto_sap_r IS RECORD
(
NU_CARTEIRA_VERSION  VND.ELO_CARTEIRA_SAP.NU_CARTEIRA_VERSION%TYPE,
CD_SALES_ORG VND.ELO_CARTEIRA_SAP.CD_SALES_ORG%TYPE,
CD_PRODUTO_SAP VND.ELO_CARTEIRA_SAP.CD_PRODUTO_SAP%TYPE,
NO_PRODUTO_SAP VND.ELO_CARTEIRA_SAP.NO_PRODUTO_SAP%TYPE

);

TYPE elo_produto_sap_t IS TABLE OF elo_produto_sap_r
INDEX BY PLS_INTEGER;
tof_produto_sap elo_produto_sap_t;


CURSOR C_CENTRO_EXPEDIDOR (PI_NU_CARTEIRA_VERSION VND.ELO_CARTEIRA_SAP_IMPORT.NU_CARTEIRA_VERSION%TYPE) IS 
SELECT 
SAP.NU_CARTEIRA_VERSION,
SAP.CD_CENTRO_EXPEDIDOR ,
NVL(CE.DS_CENTRO_EXPEDIDOR, SAP.DS_CENTRO_EXPEDIDOR) DS_CENTRO_EXPEDIDOR
FROM VND.ELO_CARTEIRA_SAP SAP
LEFT JOIN CTF.CENTRO_EXPEDIDOR CE 
ON CE.CD_CENTRO_EXPEDIDOR = SAP.CD_CENTRO_EXPEDIDOR

WHERE SAP.NU_CARTEIRA_VERSION = PI_NU_CARTEIRA_VERSION
GROUP BY SAP.NU_CARTEIRA_VERSION, SAP.CD_CENTRO_EXPEDIDOR, 
CE.DS_CENTRO_EXPEDIDOR, SAP.DS_CENTRO_EXPEDIDOR
;




TYPE elo_centro_expedidor_r IS RECORD
(
NU_CARTEIRA_VERSION             VND.ELO_CARTEIRA_SAP.NU_CARTEIRA_VERSION%TYPE,
...
TYPE elo_centro_expedidor_t IS TABLE OF elo_centro_expedidor_r
INDEX BY PLS_INTEGER;
tof_centro_expedidor elo_centro_expedidor_t;
...
NULL DS_CENTRO_EXPEDIDOR, SPA.DH_CARTEIRA,
...
NULL CD_SALES_DISTRICT, 
SPA.CD_INCOTERMS,
NULL CD_SALES_OFFICE,
NULL NO_SALES_OFFICE,
SPA.CD_SALES_GROUP,
NULL NO_SALES_GROUP,
...
NULL NO_SALES_DISTRICT,

NULL DS_CENTRO_EXPEDIDOR_FIX,
NULL CD_CENTRO_EXPEDIDOR_FIX
...
--V_NU_CARTEIRA_VERSION:=P_NU_CARTEIRA_VERSION;

    BEGIN 
    open VERS (P_NU_CARTEIRA_VERSION );
    ...
    BEGIN
    V_TRAVA:='S';
    RAISE_APPLICATION_ERROR(-20001, 'ERRO ENCONTRADO: GX_ELO_BATCH_ISSUE.301 - ' || SQLCODE || ' -ERROR- ' || SQLERRM);
    --ROLLBACK;
    END;
    ...
    BEGIN 
    OPEN    C_CENTRO_EXPEDIDOR (P_NU_CARTEIRA_VERSION);                               
    FETCH   C_CENTRO_EXPEDIDOR BULK COLLECT INTO tof_centro_expedidor LIMIT V_LIMIT;
    CLOSE   C_CENTRO_EXPEDIDOR;
    EXCEPTION  
    WHEN OTHERS THEN 
    BEGIN
     V_TRAVA:='S';
    ...
    BEGIN 
    OPEN    C_PRODUTOSAP (P_NU_CARTEIRA_VERSION);                               
    FETCH   C_PRODUTOSAP BULK COLLECT INTO tof_produto_sap LIMIT V_LIMIT;
    CLOSE   C_PRODUTOSAP;
    EXCEPTION  
    WHEN OTHERS THEN 
    BEGIN
     V_TRAVA:='S';
    RAISE_APPLICATION_ERROR(-20001, 'ERRO ENCONTRADO: GX_ELO_BATCH_ISSUE.300 - ' || SQLCODE || ' -ERROR- ' || SQLERRM);
    --ROLLBACK;
    END;
    ...
    BEGIN 
    OPEN    C_SALES_STRU (P_NU_CARTEIRA_VERSION);                               
    FETCH   C_SALES_STRU BULK COLLECT INTO tof_sales_group LIMIT V_LIMIT;
    CLOSE   C_SALES_STRU;
    EXCEPTION  
    WHEN OTHERS THEN 
    BEGIN
    ...
    IF tof_elo_sap.COUNT > 0 and V_TRAVA='I' THEN 
    BEGIN 
        ...
        begin 
            ...
            V_NU_CARTEIRA_VERSION:=tof_elo_sap(i_ctupdate).NU_CARTEIRA_VERSION;
            
            BEGIN 
            OPEN    C_WHO_UPDATE (V_NU_CARTEIRA_VERSION);                               
        ...
        IF tof_elo_carteira_sap.COUNT > 0 AND  V_TRAVA='I' THEN
        BEGIN
           ...
           values(
            ...
            BEGIN
                ...
                BEGIN 
                FORALL i_cart in INDICES OF tof_sales_group
                UPDATE VND.ELO_CARTEIRA_SAP_IMPORT 
                SET NO_SALES_GROUP = tof_sales_group(i_cart).NO_SALES_GROUP,
                NO_SALES_OFFICE = tof_sales_group(i_cart).NO_SALES_OFFICE,
                CD_SALES_OFFICE = tof_sales_group(i_cart).CD_SALES_OFFICE,
                NO_SALES_DISTRICT = tof_sales_group(i_cart).NO_SALES_DISTRICT,
                CD_SALES_DISTRICT = tof_sales_group(i_cart).CD_SALES_DISTRICT
                WHERE 
                NU_CARTEIRA_VERSION = tof_sales_group(i_cart).NU_CARTEIRA_VERSION
                AND CD_SALES_GROUP = tof_sales_group(i_cart).CD_SALES_GROUP;
                COMMIT;
                ...
                END;
                END;  
                tof_sales_group.DELETE;  
                
                BEGIN 
                FORALL i_cart in INDICES OF tof_produto_sap
                UPDATE VND.ELO_CARTEIRA_SAP_IMPORT 
                SET 
                NO_PRODUTO_SAP = tof_produto_sap(i_cart).NO_PRODUTO_SAP
                WHERE 
                NU_CARTEIRA_VERSION = tof_produto_sap(i_cart).NU_CARTEIRA_VERSION
                AND CD_SALES_ORG = tof_produto_sap(i_cart).CD_SALES_ORG
                AND CD_PRODUTO_SAP = tof_produto_sap(i_cart).CD_PRODUTO_SAP;
                ...
                END;
                END;   
                tof_produto_sap.DELETE;
                            
                       
                BEGIN 
                FORALL i_cart in INDICES OF tof_centro_expedidor
                UPDATE VND.ELO_CARTEIRA_SAP_IMPORT 
                SET 
                DS_CENTRO_EXPEDIDOR = tof_centro_expedidor(i_cart).DS_CENTRO_EXPEDIDOR
                WHERE 
                NU_CARTEIRA_VERSION = tof_centro_expedidor(i_cart).NU_CARTEIRA_VERSION
                AND CD_CENTRO_EXPEDIDOR = tof_centro_expedidor(i_cart).CD_CENTRO_EXPEDIDOR;
...
and nu_carteira_version = '20180112170149'
...
delete ELO_CARTEIRA_SAP_IMPORT
where nu_carteira_version = '20180321173601'
and cd_elo_carteira_sap = 313961;
